...
function sanitizeHuntDetailsForClient(details: HuntDetails | null): Partial<HuntDetails> | null {
  ...
  if (details.state === HuntState.ACTIVE || 
    ...
    console.log('treasurePositionX', treasurePositionX, treasurePositionY, salt);
    return rest;
  }
        ...
        if (initialHuntDataForNftGeneration && (initialHuntDataForNftGeneration.state === HuntState.WON || initialHuntDataForNftGeneration.state === HuntState.LOST) && !initialHuntDataForNftGeneration.nftImageIpfsCid && !initialHuntDataForNftGeneration.nftMetadataIpfsCid) {
            ...
            try {
                ...
                try {
                    ...
                    imageResponse = await fetch(ogImageUrl, { signal: controller.signal });
                    ...
                    imageResponse = null; // Ensure it's null so we skip pinning
                }
                // --- End timeout for fetch ---

                // Proceed only if imageResponse was successful
                if (imageResponse) {
                    ...
                    console.warn(`[NFT Generation Post-Tx] Skipping IPFS pinning due to fetch failure for OG image for hunt ${huntId}.`);
                }
